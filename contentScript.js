...
function asda() {
    ...
    pPerso = "Subtotal:  <b>"+subtotal[0].innerText +"</b><br>ASDA Standard Delivery: <b>£4.00</b>";
    deliveryTotal = [4.00];
...
function george() {
    ...
    //Colour
    var productColor = document.querySelectorAll("div.attributes-container > div:nth-child(1) > span.attribute-value");
    for (var i = 0; i < productColor.length / 2; i++) {
        pColor += productColor[i].textContent + n;
    }
    console.log(productColor);
    ...
    //size
    var productSize = document.querySelectorAll("div.attributes-container > div:nth-child(2) > span.attribute-value");
    for (var i = 0; i < productSize.length / 2; i++) {
    ...
    console.log(productPrice[3].childNodes[1].lastChild);
    console.log(test);
    chrome.runtime.sendMessage({ price: pPrice });
...
function MandSDelivery() {
    ...
    delivery[1].click();
    setTimeout(() => markAndSpencers(), 1000)
}
//marks&spencers
...
function zara() {
    ...
    //products
    var productName = document.getElementsByClassName("shop-cart-item-header__description");
    for (var i = 0; i < productName.length; i++) {
        pName += productName[i].textContent + n;
    }
    console.log(pName);
    ...
    //Quantity & number of items
    var productQuantity = document.querySelectorAll("div.shop-cart-item__secondary-actions > div > div");
    console.log(productQuantity);
    for (var i = 0; i < productQuantity.length; i++) {
        pQuty += productQuantity[i].textContent + n;
        totalItems.push(parseInt(productQuantity[i].textContent, 10));
